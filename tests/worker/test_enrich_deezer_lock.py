"""
Tests for the enrich_catalog (Deezer) single-instance Redis lock.

Twin of test_beatport_lock.py: since A3-03 the Deezer sweep is hardened like
its Beatport sibling — a Redis lock (SET NX EX) prevents two instances (beat
run vs admin trigger, or broker re-delivery) from enriching the catalog
concurrently, and its TTL must stay above the task's time_limit so the lock
...
# test_beatport_lock.py — setdefault keeps this idempotent across files)
_MOCK_MODULES = [
...
# Same attributes as test_task_refactor.py's decorator: the files overwrite the
# shared workers.celery_app mock, so whichever import order pytest picks, tasks
# decorated by either mock must satisfy every test file's assertions
...
class TestEnrichCatalogDeezerLock:
    def test_skips_when_lock_held(self, catalog_mod, fake_redis, fake_self, monkeypatch):
        run = MagicMock()
        monkeypatch.setattr(catalog_mod, "_run_enrich_catalog", run)
    ...
    def test_acquires_runs_and_releases(self, catalog_mod, fake_redis, fake_self, monkeypatch):
        run = MagicMock(return_value={"enriched": 42, "errors": 0, "merged": 1})
        monkeypatch.setattr(catalog_mod, "_run_enrich_catalog", run)
        ...
        assert result == {"enriched": 42, "errors": 0, "merged": 1}
        run.assert_called_once_with(fake_self)
        _, kwargs = fake_redis.set.call_args
        assert kwargs.get("nx") is True
        assert kwargs.get("ex") == catalog_mod.DEEZER_LOCK_TTL
    ...
    def test_releases_lock_on_failure(self, catalog_mod, fake_redis, fake_self, monkeypatch):
        ...
        with pytest.raises(RuntimeError):
            catalog_mod.enrich_catalog(fake_self)
    ...
    def test_does_not_release_lock_it_no_longer_owns(
        ...
        run = MagicMock(return_value={"enriched": 1, "errors": 0, "merged": 0})
        monkeypatch.setattr(catalog_mod, "_run_enrich_catalog", run)
        fake_redis.set.return_value = True
    ...
    def test_lock_ttl_covers_task_time_limit(self, catalog_mod):
        ...
        must stay ≥ the task's time_limit. The Deezer sweep is a single nightly
        pass with a long time_limit (9000s), so its lock TTL sits just above it."""
        assert (
            catalog_mod.DEEZER_LOCK_TTL
            >= catalog_mod.enrich_catalog.time_limit
        )
