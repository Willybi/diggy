...
def resolve_set_tracks(self):
    ...
    sys.path.insert(0, "/app")
    from deezer_enrich import _ensure_bucket, _get_s3
                        ...
                        if dz_entries:
                            for i in range(0, len(dz_entries), 100):
                                ...
                                stats = await enrich_deezer_batch(
                                    session, batch, pool, s3, existing_isrcs
                                )
                ...
                async def _fetch_one(artist):
                    ...
                    if img_data:
                        ...
                        if upload_image_bytes_to_bucket(
                            s3, img_data, f"{artist.id}.jpg", ARTIST_BUCKET
                        ):
...
@celery_app.task(name="workers.tasks.compute_trends", bind=True)
