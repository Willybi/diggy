...
_SOURCE_CONFIG = {
    ...
    # Preview MP3 downloads off the Deezer CDN (cdnt-preview.dzcdn.net), used
    # ONLY by the BPM analysis drain. Throttled independently of the "deezer"
    # API window: the CDN 429-rate-limits the VPS IP on SUSTAINED download rate
    # (measured 2026-08-23: ~60% 429 at ~3 req/s [nightly], but only ~6% at
    # ~0.86 req/s → the throttle is rate-avoidable, not a hard per-run cap).
    # Rate/concurrency are env-tunable (no deploy) to dial in the CDN sweet spot;
    # default ~1 req/s. Local bucket only (BPM is single-instance).
    "deezer_preview": (
