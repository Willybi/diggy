...
def _levenshtein_ratio(a: str, b: str) -> float:
    """Character-level edit-distance similarity: 1 - levenshtein / max(len).

    A CHARACTER distance (unlike ``token_set_ratio``, which is token-based) — it
    catches near-identical titles that differ only in spacing/punctuation runs
    ("The Lot Radio" vs "TheLotRadio"), where two spellings tokenise apart yet
    are a couple of edits apart. Returns 1.0 when both strings are empty.
    """
    if not a and not b:
        return 1.0
    m, n = len(a), len(b)
