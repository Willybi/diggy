  ...
  <div class="player">
    <div class="pl-shell">
      ...
      <button
        ...
        @click="player.toggle()"
      >
        <svg v-if="!player.playing" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div class="pl-stats">
        <div class="pl-stat">
          <span class="pl-stat-val">{{
            player.track?.bpm ? Math.round(player.track.bpm) : '—'
          }}</span>
...
function onRailKey(e) {
  ...
  switch (e.key) {
    case 'ArrowRight':
      player.seek(Math.min(player.currentTime + step, player.duration))
      break
    case 'ArrowLeft':
      player.seek(Math.max(player.currentTime - step, 0))
      break
    case 'Home':
      player.seek(0)
      break
