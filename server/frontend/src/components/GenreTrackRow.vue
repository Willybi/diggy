<template>
  <div class="track-row" :class="{ playing: isPlaying }" @click="onPlay">
    ...
    <!-- Cover -->
    <div class="cover">
      <img v-if="track.hasArtwork" :src="`/storage/catalog-artworks/${track.id}.jpg`" alt="" loading="lazy" @error="e => e.target.style.display = 'none'" />
...
.track-row.playing { background: var(--accent-wash, var(--surface-2)); }
...
.fb { font: 600 14px/1 var(--font-ui); color: var(--ink-3); }
...
.bpm { color: var(--ink-2); min-width: 30px; text-align: right; }
.key { color: var(--accent-ink); min-width: 28px; text-align: center; }
.dur { color: var(--ink-3); min-width: 36px; text-align: right; }
