...
**Statut : EN COURS — C13.a (persistance des signaux) livree 2026-09-08 (1a41adb/91bf83e : migration 0056 sets.channel/styles + funnel import_audiostream + affichage front ; script backfill_set_signals.py livre, run OPS a confirmer) ; C13.b outillage amorce (scripts/local/trackid_titles/), GATE + C13.c/d a faire. Issu de la consultation « infos TrackID » (2026-09). Absorbe le reliquat « Enrichissement set→artiste (D6 Sets liste) ». A risque prod modere (migration + liens SetArtist).**
...
- [ ] Backfill OPS des ~42k sets existants : `channel`/`styles` sont deja dans `trackid_index` → **report SQL sans re-crawl reseau** (join `sets.external_id = trackid_id::text`). Les signaux uniquement dans le detail (`referenceCount`…) necessiteraient un re-crawl → hors C13.a (a arbitrer separement, cf. C12 Exigence B).
...
- [ ] Miner les ~381k titres de `trackid_index` : tokeniser, remplacer chaque brique reconnue par un placeholder (`DATE`, `EP`, `FORMAT`, `DELIM`, `?`), **compter les squelettes** resultants (top ~40 = X % de couverture). Sortie : distribution + echantillons par squelette.
- [ ] Constituer un **jeu de test etiquete a la main** (echantillon de titres → artiste(s) attendu(s)) pour mesurer precision/rappel.
- [ ] **GO/NO-GO** : si le deterministe seul couvre ~85 % → pas de LLM. Si ~60 % avec une longue traine riche en artistes reels → le LLM devient interessant. **La necessite du LLM se decide sur ces chiffres, pas a priori.**

### C13.c — Extracteur deterministe (approche SOUSTRACTIVE, pas generative)

- [ ] Ne PAS enumerer les formats de titres complets (explosion combinatoire) → reconnaitre les **~20 types de briques** ; n'importe quel ordre est gere automatiquement.
- [ ] **Strippers de bruit** (petits, independants, testes unitairement) : dates (toutes formes : `05/09/2025`, `05092025`, `0509`, `20th Feb 2032`, `June 2026`, ISO…), marqueurs episodiques (`EP\d+`, `SET\d+`, `#\d+`, `Vol.\d+`, `Part \d+`, `\d{3,}` isole), mots de format (`DJ Set`, `Live`, `DJ Mix`, `Podcast`, `Guest Mix`, `Exclusive`, `FREE DOWNLOAD`, `Official`…), lieu/festival (gazetteer, best-effort).
- [ ] **Connecteurs qui STRUCTURENT les artistes** (baliser, pas retirer) : `B2B`/`B3B`/`F2F`/`VS`/`X`/`&`/`feat.`/`ft.` → artistes autour + separation multi-artistes. Parentheses/crochets classes par contenu (date/lieu dedans = bruit ; nom dedans = a garder).
- [ ] **Robustesse au format** : ne PAS gater sur « le titre commence par le nom » (perd `"Brooklyn - 20 juin - Artiste"`). Deux voies : (1) si `channel` matche un artiste reel → lien (independant du format du titre) ; (2) sinon, scan de tout le titre foldé pour un artiste connu, ou qu'il soit. Le « debut de titre » n'est qu'un **booster de confiance**, jamais un filtre.
- [ ] **Filet fail-safe (invariant #4)** : l'extracteur PROPOSE, la verif DISPOSE. Tout candidat est confirme contre Deezer (matchers fold accents/ponctuation + plancher de fans, cf. X4) et/ou les artistes connus + `channel` AVANT tout lien `SetArtist`. Une extraction ratee → charabia → aucun match → **pas de lien** (l'erreur coute du rappel, pas de la precision). Reutiliser `resolve_name_to_id` (lookup-only, extrait de `get_or_create_artist`, cf. C12 Exigence B).
- [ ] Cabler la derivation au beat / au funnel de sets (contrairement a `link_set_artists` qui ne tourne qu'en manuel), ou remplacer/ameliorer `link_set_artists`.
...
- [ ] `sets.channel` + `sets.styles` peuples (funnel + backfill des ~42k existants) et exposes cote UI.
- [ ] Corpus mining livre (distribution des squelettes + jeu de test) + decision GO/NO-GO LLM tracee sur des chiffres.
- [ ] Derivation d'artiste de set **automatique** (dans le beat/funnel) avec precision mesuree sur le jeu de test, liens `SetArtist` crees sous verif Deezer (0 faux positif tolere par design, invariant #4) → part des sets a DJ cliquable en forte hausse vs les ~60 actuels.
