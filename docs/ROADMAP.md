...
**Statut : TERMINE (2026-08-30 ; 9566859, deploy_verify SAIN). Implementation des exigences A livree+deployee (8 lots) : enrich_priority + migration 0051 ; gate Beatport Tier-1-only (retries jamais floores, Deezer intact) ; verrou 2 stamping (phase du set / FLUX_PRIORITY=100, MAX-merge) ; swap curseur (backfill consomme trackid_index par score + beat 02h re-active) ; outil scoring local + import OPS --scores ; capture detail (migration 0052) ; alerte flux>budget. Pre-vol shadow-hydration complet -> net-new EXACT 1,15M, scope fige = phases 0-3 (~297k sets, ~1,1M titres, ~111j Beatport). Feature INERTE tant que l'OPS import scores (dump -> --apply) n'est pas lance ; cap bas au demarrage (lecon hostinger-cpu-throttle). DIFFERE : C.1 recrawl polling hitRate + graphe backlog front. Brief : `docs/prompts/C12-priorisation-backfill.md` ; spec pre-vol : `docs/prompts/C12-scoring-preflight-results.md`. A risque prod reel.**
