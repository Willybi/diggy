...
| **Infra test front : vitest en pool parallèle casse sur Windows** (`document is not defined` — jsdom non initialisé dans les workers) : validation locale possible seulement en single-fork, masque de potentielles régressions en local (CI Linux non affectée). + Prettier hors du gate `npm run lint` (eslint seul) → des lignes déjà committées non conformes Prettier passent. | Opportuniste — hygiène outillage test |
