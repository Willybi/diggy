...
----  ------------------------------------  ----------  ----------   ------
 ...
 C14  Nouvelles sources de sets & veille artistes (YT/SC) HAUT 10-15 jours A FAIRE — cadre 2026-09-18 (session produit + chiffres prod du jour) : decouverte CHANNEL-CENTRIC (entite channel + veille RSS YouTube/SoundCloud + sets metadata-only + extraction artiste C13 reutilisee) ∥ veille tracks Deezer sur cohorte derivee ~3,4k artistes + backfill discographie LOCAL gate par benchmark ∥ tracklists communautaires (description + commentaires timecodes, gate corpus). Decisions : PAS de TrackID Premium ; sets sans tracklist acceptes. Absorbe C6.d (suivi SoundCloud)
...
**Statut : A FAIRE — cadre 2026-09-18 (session de cadrage produit, chiffres verifies en prod le jour meme).**
...
- [ ] Materialiser la **cohorte derivee** (signaux ci-dessus) avec tiers de cadence (quotidien ~3,4k / hebdo / mensuel) + recalcul periodique (auto-promotion) + surface admin (voir la liste, pin/exclude, tier force).
- [ ] Brancher le **check releases Deezer** sur la cohorte au lieu des seuls follows — machinerie `check_followed_artists` + `_crawl_track` REUTILISEE (feed Nouveautes inchange) ; budget par tier (~3,4k/j ≈ 56 min a 1 req/s en fenetre nocturne, ordre de grandeur `link_artists_deezer` → VPS OK).
...
- [ ] Cohorte derivee materialisee (admin : liste, tiers, pin/exclude) + veille tracks Deezer active sur ~3,4k artistes — le feed Nouveautes alimente au-dela des 13 follows.
