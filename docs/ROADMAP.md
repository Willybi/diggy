...
**Statut : C14.a Phase 1 TERMINE + DEPLOYE 2026-09-22 (918c94e, /deploy_verify SAIN) — table `artist_cohort` (migration 0060) + recompute nightly (beat 04:15) + `check_followed_artists` branche sur la cohorte DUE (`select_due_cohort_ids`, budget-cap `COHORT_WATCH_NIGHTLY_BUDGET` + garde deadline AV9 ; `_check_new_sets` reste sur les follows) + admin `/admin/cohort` (`cohort_service` list/override) + onglet AdminCohort (7e ; pin/exclude/tier force + un-force via `forced_tier:null`). 1er recompute prod OK : T1=3469, T2=18336, total 21805. Brief `docs/prompts/C14a-veille-cohorte-deezer.md`. RESTE : C14.a Phase 2 (backfill discographie Deezer, outil LOCAL, GATED par benchmark) + C14.b + C14.c. Cadre 2026-09-18 (session de cadrage produit, chiffres verifies en prod le jour meme).**
...
- [ ] **Veille** : flux **RSS YouTube par chaine** (gratuit, sans quota — la veille de 2-3k chaines = ~1 h de HTTP leger/jour) + API SoundCloud (cle self-serve — demande a deposer immediatement) ; filtre duree >30-40 min = « c'est un set » ; anti-doublon vs `trackid_index`/`sets` AVANT creation.
- [ ] **Sets metadata-only** : `source='youtube'/'soundcloud'` (aujourd'hui `sets.source` est toujours `'trackid'` — auditer les lecteurs), chaine C13 appliquee au titre (event_date, channel_canonical, extraction artiste + verif Deezer — REUTILISEE TELLE QUELLE, elle est source-agnostique) ; etats UI (« sans tracklist » / « tracklist partielle ») ; exclusion similarite/scoring tant que 0 track liee ; **reconciliation `match_set` si le set apparait plus tard sur TrackID (la version TrackID gagne)** ; dedup cross-source ULTRA-CONSERVATRICE (prudence C6/C11, invariant #4).
...
- [ ] N chaines surveillees (seed + curation lieux) ; des sets YT/SC entrent en base en metadata-only avec artiste extrait/verifie — **le cas Schrotthagen (set Ritter Butzke) DOIT etre rattrape par le mecanisme, c'est le test d'acceptance**.
