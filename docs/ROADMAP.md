...
- [x] Injecter le score contenu comme SECOND axe dans `similarity_service`/`recommendation_service` (ponderation a calibrer ; golden tests d'ancrage comme le fix pooling C4) — LIVRE 2026-09-21 en RETRIEVAL-FIRST (bonus additif CONTENT_BONUS sur univers borne KNN ∪ co-occ, golden tests intacts ; ponderation a figer via eval_hybrid, run operateur)
