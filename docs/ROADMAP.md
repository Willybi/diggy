...
----  ------------------------------------  ----------  ----------   ------
 ...
 C9   Embeddings audio & reco par contenu   BAS         8-12 jours   TERMINE (2026-09-21 ; C9.a/b/c livres+deployes, C9.d hors DoD/stretch) — C9.a socle pgvector (migration 0049, Discogs-EffNet fige v1) + backfill BOUCLE 290 594 embeddings (~99,5% des previewables, residu 1413 irreductible) + eval a l'echelle (docs/c9-benchmark/, cross-artist lift@10 62,2x) ; C9.b shelf « Sonne comme » PUBLIQUE sur Track Detail (GET /api/catalog/{id}/content-similar, catalog_visible ; gate admin retire 2bdb27f) ; C9.c reco HYBRIDE retrieval-first en prod (scan full-pool ~700k SUPPRIME, load_candidate_pool retire, KNN pgvector ∪ co-occ inversee par seed re-ranke C2, bonus contenu ADDITIF CONTENT_BONUS fige 0.5) ; commits dd0ae61->2bffd1a puis 598441e, 155 tests verts ; /deploy_verify SAIN. GATE BENCHMARK C9.0 PASSE 2026-08-21 (GO FRANC, lift@10 cross-artist 32.5x). RESTE OPS non bloquant : salves de couverture embeddings + suivi qualitatif « Pour toi »
 ...
 C10  Pool similarite precalcule (nightly)  BAS         3-5 jours    CADUC — supersede par C9.c retrieval-first (2026-09-21) : le full-pool par requete que C10 devait cacher a ete SUPPRIME (load_candidate_pool retire, retrieval borne par seed). Plus d'objet ; declenche puis annule le meme jour
 ...
 D11  Admin — Refonte graphique (design)     BAS         3-5 jours    TERMINE (2026-08-31 ; f57956a + 506edc4 + 86450a7, /deploy_verify SAIN) — pipeline /refonte_page complet (fiche §8 → handoff Claude Design → work_manager 8 lots + correctifs → deploy → revue design soldee + 1 regression hover corrigee) ; pur habillage front (0 back) : 6 archetypes A→F + Apercu harmonise, NOUVEAU socle admin-table.css (.at-*) + AdminIcon (12 glyphes SVG, 0 emoji), courbes sur tokens --chart-* (correctif D22), pagination serveur set-flags (param offset existant), survol des courbes restaure. 66 composants front, 741 tests verts.
 C11  Index TrackID (listing global)      MOYEN       3-5 jours    TERMINE (2026-08-27 ; ff0429c, deploy_verify SAIN, import OPS applique) — index exhaustif des ~381k sets TrackID via le listing /audiostreams (spider LOCAL 1 req/s, table public.trackid_index) + rapport volumetrie. Etape 0 PREALABLE : arret du backfill chronologique (redis trackid_backfill_done=1 immediat + retrait entree beat backfill-trackid-sets). Prealable de C12. Brief docs/prompts/C11-index-trackid.md
 C12  Priorisation backfill TrackID       MOYEN       5-7 jours    TERMINE (2026-08-30 ; 9566859, deploy_verify SAIN) — implementation livree+deployee (8 lots) : colonne catalog.enrich_priority (migration 0051, SmallInt + index partiel) ; gate Beatport select_enrich_candidates(order_by_priority/priority_floor) sur le Tier-1 SEUL (retries 30/90j jamais floores/reordonnes, Deezer intact, mirror count_enrich_backlog, coalesce NULL->baseline 75, floor opt-in ENRICH_PRIORITY_FLOOR off par defaut) ; verrou 2 stamping (resolve_set_tracks pose enrich_priority = phase du set source, FLUX_PRIORITY=100 pour un set live non score, MAX-merge cross-set/run) ; swap curseur (backfill_trackid_sets consomme trackid_index WHERE not_hydrated AND score IS NOT NULL ORDER BY score DESC, marque hydrated au fil de l'eau, beat backfill-trackid-sets-daily RE-ACTIVE 02h, no-op tant qu'aucun score importe) ; outil scoring LOCAL scripts/local/trackid_spider/score.py (phases lib/genre>=10/channel -> NDJSON, tie-break net-new intra-phase) + import OPS import_trackid_index.py --scores (UPDATE score/score_components sans toucher l'upsert miroir) ; capture detail (migration 0052 : set_tracks.label/end_time_ms + sets.can_reprocess) ; alerte flux>budget (snapshot_backlogs + Sentry). Pre-vol shadow-hydration COMPLET (364,9k sets fetches a 6 req/s, tracklists matchees read-only) : net-new EXACT 1,15M (catalog x4,3, dont 683k artiste connu ; l'extrapo 5k ~2,1M sur-projetait x2), scope fige = phases 0-3 (~297k sets, ~1,1M titres, ~111j Beatport). 2249 back verts, ruff clean, schema doc regenere. OPS RELIQUAT SOLDE (verifie prod 2026-09-04) : scores importes (293,7k not_hydrated porteurs de score sur 338,3k / 43,2k hydrated), drain backfill_trackid_sets score-based tourne chaque nuit (imported 600/nuit, deadline_hit=false), cap remonte 200->600 sans throttle, enrich_priority stampe sur 96,4k catalog (18,1k a FLUX_PRIORITY=100). DIFFERE : C.1 recrawl polling hitRate (root cause CONFIRMEE 2026-09-04 : `completion_pct=(total-unidentified)/total` est is_id-only, or set_tracks ne stocke QUE des tracks identifiees [is_id=true : 1 ligne sur 705k] -> denominateur = tracks detectees, jamais le vrai total -> ~96% des sets a 100% [42905/44202] alors que trackid_index.track_hit_rate/time_hit_rate [peuple a 100%, moyenne 0,53, 98,6% des sets < 0,95] dit ~53% identifie ; fix = re-baser sur trackHitRate/timeHitRate du listing) + graphe backlog front AdminMonitoring. Brief docs/prompts/C12-priorisation-backfill.md + spec pre-vol docs/prompts/C12-scoring-preflight-results.md
 C13  Signaux de set & extracteur d'artiste MOYEN     6-9 jours    TERMINE (2026-09-10) — voir la fiche C13 ci-dessous (migrations 0056/0057, linker C13.c au beat, backfill local ~34,2k sets a DJ)
 C14  Nouvelles sources de sets & veille artistes (YT/SC) HAUT 10-15 jours EN COURS — C14.a Phase 1 (veille Deezer sur cohorte derivee) TERMINE + DEPLOYE 2026-09-22 (918c94e, /deploy_verify SAIN) : table `artist_cohort` (migration 0060) + recompute nightly (beat 04:15, auto-promotion) + `check_followed_artists` branche sur la cohorte DUE (budget-cap + garde deadline AV9 ; T1 quotidien 3469 / T2 hebdo 18336, total 21805) + admin `/admin/cohort` (7e onglet AdminCohort : pin/exclude/tier force/un-force). **C14.b tranche « YouTube-first » TERMINE + DEPLOYE 2026-09-23** (437b5bc decouverte + 3c7d4f3 nom + aacd36c recherche/lien YouTube + 90e71bc pre-selection candidats cachee Redis + 06be0bc fix pipeline nginx ; /deploy_verify SAIN x5, **test d'acceptance Schrotthagen VALIDE en prod** : set YT cree + artiste 46469 lie, 197 sets Ritter Butzke importes) : entite `channels` (migration 0061) + client YouTube (RSS Atom defusedxml + Data API) + veille nocturne `crawl_youtube_channels` (01h30) + backfill one-shot `backfill_youtube_channel` (Data API uploads, dispatche a l'ajout) + 8e onglet AdminChannels (liste/candidats seed trackid_index/add-by-URL/recherche+pre-selection cachee) + source-awareness (`_select_unlinked_sets`+feed Nouveautes+`set_service`) + fix pipeline deploy (restart nginx conditionnel si `server/nginx/` change). Differes : resolution artiste->chaine EN MASSE (Wikidata P2397/MusicBrainz), volet SoundCloud (cle self-serve), reconciliation `match_set`, affinage extraction C13 sur titres YouTube. RESTE : C14.a Phase 2 (backfill discographie Deezer, outil LOCAL, GATED par benchmark) + C14.c (tracklists communautaires). Cadre 2026-09-18 (session produit + chiffres prod du jour) : decouverte CHANNEL-CENTRIC (entite channel + veille RSS YouTube/SoundCloud + sets metadata-only + extraction artiste C13 reutilisee) ∥ veille tracks Deezer sur cohorte derivee ~3,4k artistes + backfill discographie LOCAL gate par benchmark ∥ tracklists communautaires (description + commentaires timecodes, gate corpus). Decisions : PAS de TrackID Premium ; sets sans tracklist acceptes. Absorbe C6.d (suivi SoundCloud)
 D12  Pre-ecoute Beatport universelle (bouton + overlay embed) MOYEN 2-3 jours TERMINE (2026-09-22 ; v1 overlay c2391a0 → v2 barre dockee 30df211 → v3 carte « remplace + chip reprendre » 6f277bc reskin Claude Design, + 9feeacd chore CI retry smoke ; /deploy_verify SAIN ×3, checklist humaine validee) — partout ou un bouton play Deezer existe, une track `!has_preview && beatport_id` (75 171 lignes = 10,7 % du catalog → couverture pre-ecoute 79 %→~90 %) affiche un bouton play VARIANTE Beatport ouvrant la carte flottante BeatportBar (embed officiel, seul moyen licencie, iframe boite noire, PAS d'autoplay) ; encart inline de Track Detail remplace par le meme bouton ; ouvrir la carte PAUSE Deezer et MASQUE sa carte (jamais 2 audios ni 2 boites, chip « Reprendre la preview Deezer ») ; l'auto-advance de la file SKIPPE les Beatport-only ; priorite Deezer si les 2 existent. Quasi front-only, AUCUN modele ni migration. Residuels en fiche
```
...
----  ---------------------------------  ------
 ...
 C10  Pool similarite precalcule (nightly)      CADUC (2026-09-21 ; jamais construit — devenu caduc, supersede par C9.c retrieval-first : le full-pool par requete que C10 devait cacher a ete SUPPRIME [load_candidate_pool retire, retrieval borne par seed] ; declenche puis annule le meme jour)
```
...
### C6.d — Suivi d'artistes v2 (Soundcloud) — ABSORBE par C14

Extension du suivi artiste a Soundcloud. **Reporte apres validation de C6.c** car le scraping Soundcloud est fragile (pas d'API officielle, anti-bot). **ABSORBE par C14 (cadrage 2026-09-18)** : la veille SoundCloud/YouTube y est reprise et elargie (entite channel de 1re classe, decouverte channel-centric, cohorte derivee au lieu des seuls follows) — et l'API SoundCloud a partiellement rouvert en 2026 (self-serve keys), ce qui leve la reserve « pas d'API officielle ».
...
**Statut : TERMINE 2026-09-21 (C9.a/b/c livres ; C9.d hors DoD, optionnel — reste OPS : salves locales de couverture embeddings ; CONTENT_BONUS FIGE 0.5 le 2026-09-21 apres run eval_hybrid). Cadre le 2026-07-31. Horizon recherche assume (C9.d). GATE BENCHMARK C9.0 PASSE 2026-08-21 : GO FRANC (lift@10 cross-artist 32.5x, voir docs/c9-benchmark/) — C9.a/b/c debloques, C9.d degrade en stretch. C9.a TERMINE 2026-08-29 : socle pgvector deploye (migration 0049, modele Discogs-EffNet fige v1) + backfill local BOUCLE (290 594 embeddings prod = 99,52% des previewables, residu 1413 irreductible ; fil de l'eau VPS differe car EffNet trop lourd) + knob EMBED_MIN_INTERVAL commite (358fb54 : le throttle Deezer est pilote par le debit, pas par --workers → passe douce 1 rps + cooldown leve le rate-limit) + monitoring couverture embeddings livre 2026-08-24 (1987fa3 : bloc payload embeddings covered/eligible/missing + StatTile % + split du burn-down en 3 graphes thematiques) + eval a l'echelle VALIDEE (`eval_at_scale.py`, 3000 sets / 40 561 tracks : cross-artist lift@10 = 62,2x, shuffled 1,03x, signal confirme au reel). C9.b LIVRE en mode ADMIN-ONLY 2026-08-24 : Lot 1 back (d3dad75) = endpoint GET /api/catalog/{id}/content-similar (KNN cosine pgvector via comparator EmbeddingVector.cosine_distance, compose avec catalog_visible + cache Redis, [] si seed sans embedding), valide en prod (voisins coherents, diversite d'artistes) ; Lot 2 front (45e4559) = shelf « Sonne comme » sur Track Detail gate admin (v-if auth.user?.is_admin + garde reseau, sans score expose, masque si vide) ; les deux /deploy_verify SAIN. Passage PUBLIC LIVRE 2026-09-21 (2bdb27f, couverture 92% des previewables — salves L0 en cours pour le residu + l'inflow drain VPS). C9.c TERMINE cote code 2026-09-21 (chantier /work_manager 5 lots, commits dd0ae61/eccbe38/23219e9/2bdb27f/cca1816) : RETRIEVAL-FIRST — le scan full-pool (~700k lignes, 85,6 s/compute reco, /similar 10-20 s, similar_sets ~21 s a froid) est SUPPRIME de toutes les surfaces ; chaque seed retrouve un univers BORNE (KNN audio pgvector ∪ co-occurrence inversee via les index set_inverted/playlist_inverted du SimilarityContext) re-ranke par le bareme C2 ; bonus contenu ADDITIF weight_seed×CONTENT_BONUS×(1−dist) (jamais un blend, decision C9.0-bis), candidat KNN-only surface seul (cold-start teste PG), non-embedde = score meta exact, dislikes = penalite meta seule ; FRONTIERE ASSUMEE : un candidat metadonnees-seul sans co-occ ni embedding n'est plus recommandable ; load_candidate_pool+POOL_SIZE_WARN supprimes, SIMILAR_SETS_SEED_CAP restaure 6→12. Mesures prod post-deploy : /similar froid 0,54-0,68 s, /sets/similar 2,0-2,8 s a 12 seeds ; residuel ~12 s = build du contexte par worker post-deploy (cache 6 h, preexistant). Outil d'eval offline livre (worker/embedding_backfill/eval_hybrid.py + docs/c9-benchmark/EVAL_HYBRIDE.md : univers retrieval iso-prod, baseline gower_lite anti-leakage, grille --bonus-grid) — run opere le 2026-09-21 (2000 sets, univers 29 911) : hold-out co-occ MONOTONE decroissant avec le bonus (meta 173,8× > hybrid@0.5 104,7× > audio 41,4×) — metrique aveugle au cold-start (lecture EVAL_HYBRIDE.md §7), CONTENT_BONUS FIGE 0.5 (598441e, reco outil + parcimonie + « garder bas ») ; bonus du run : le retrieval L1-L3 est VALIDE (92,3% hit@50 cross-artist sur ~215 candidats/seed — l'univers borne contient les bonnes reponses). Suivi qualitatif « Pour toi » a l'usage.**
...
- [x] Backfill ~175k previews (batch, reprise, throttle) — LIVRE 2026-08-29 (290 594 embeddings prod = 99,52% des previewables, residu 1413 irreductible : ~843 no_preview genuins + ~570 HTTPError tenaces ; salves keyset locales) ; fil de l'eau post-enrichissement DIFFERE (EffNet ~8 CPU-s/track trop lourd pour le fair-use VPS au debit actuel)
- [x] Evaluation quantitative AVANT toute feature : les voisins d'embedding predisent-ils la co-occurrence en sets ? — LIVRE 2026-08-29 (`eval_at_scale.py`, echantillon 3000 sets / 40 561 tracks scorables) : SIGNAL CONFIRME A L'ECHELLE — cross-artist lift@10 = 62,2x [IC95 60,1-64,3], controle shuffled@10 = 1,03x (~1,0 = signal reel non artefactuel), hit-rate@10 15,5% / @50 35,8%. On suit l'existence/force du signal (lift >> 1, shuffled ~1), pas le lift absolu qui ne se transporte pas du dense (6740) au reel bruite (cf. RAPPORT_C9.0-bis.md §4)
...
- [x] Shelf « Sonne comme » sur Track Detail (consomme TrackCard, aucune nouvelle famille de composants attendue) — LIVRE gate admin, sans score (45e4559) ; gate LEVE → PUBLIC 2026-09-21 (2bdb27f, couverture 92%)
- [ ] (option vitrine) cartographie 2D de la bibliotheque (projection UMAP) — pas prioritaire

### C9.c — Reco hybride

- [x] Injecter le score contenu comme SECOND axe dans `similarity_service`/`recommendation_service` (ponderation a calibrer ; golden tests d'ancrage comme le fix pooling C4) — LIVRE 2026-09-21 en RETRIEVAL-FIRST (bonus additif CONTENT_BONUS sur univers borne KNN ∪ co-occ, golden tests intacts ; CONTENT_BONUS FIGE 0.5 le 2026-09-21 apres run eval_hybrid, 598441e)
- [x] Cold-start : verifier qu'un son sans historique remonte dans « Pour toi »/Radar via le seul axe contenu — LIVRE (teste PG : un candidat KNN-only sans aucune co-occurrence surface par son seul bonus contenu)
...
**Priorite : HAUT (etait BAS tant que conditionnel)**
**Estimation : 3-5 jours**
**Depend de : AV3 (palliatifs Q3a livres + mesures)**
**Statut : CADUC — SUPERSEDE par C9.c retrieval-first (2026-09-21, commits ee95b1c/2bffd1a). C10 a ete DECLENCHE le 2026-09-21 sur l'OOM full-pool, PUIS annule le meme jour : C9.c a SUPPRIME le scan full-pool de toutes les surfaces (`load_candidate_pool` retire, chaque seed retrouve un univers BORNE = KNN pgvector `content_neighbor_ids` ∪ co-occurrence inversee `cooc_candidate_ids`, charge par `load_candidates_by_ids`) — il n'y a donc PLUS de pool ~700k a precalculer/partager, la premise de C10 est void (cf. mesures post-deploy C9.c : /similar froid 0,54-0,68 s, /sets/similar 2,0-2,8 s). Un eventuel futur residu latence/RSS sera a re-cadrer sur le COUT du retrieval (KNN + co-occ), pas sur un pool a cacher — donc sans rapport avec l'ancien C10. Contexte historique du declenchement conserve ci-dessous. Le pool par requete a atteint ~699 600 candidats (inflow C12, +16k/3j, trajectoire ~1M sur le scope C12) et les deux seuils de declenchement sont tombes le meme jour : (1) vrai OOM memcg dans le cgroup api (worker uvicorn ~1,93 GiB anon-RSS tue a 12:24 UTC, cap conteneur 3G, `memory.events` max=19/oom_kill=1 — chaque requete construit SON pool par viewer, 2 computes a froid concurrents ne tiennent plus) ; (2) `GET /api/sets/86761/similar` a froid > 60 s → timeout nginx 110. Contexte : le fix ping-kill `de1ddfa` (2026-09-18, to_thread) est VALIDE (3j sans « prematurely closed ») — il a transforme les 502 en latence, C10 traite la latence/RSS a la racine. Palliatifs deja en place a integrer/depasser : `98dca02` (reco a froid deportee au worker, dispatch + « Pour toi » vide — couvre /radar/feed mais PAS /catalog/{id}/similar ni /sets/{id}/similar, restes inline) + `SIMILAR_SETS_SEED_CAP` 12→6 (2026-09-21, levier lineaire interim). Design pressenti : le pool est par-viewer (`catalog_visible`) mais sa masse est le scope `shared` identique pour tous → precalculer/partager le pool shared (nightly ou TTL en process) + overlay des lignes privees du viewer par requete ; NE PAS monter le cap api 3G→4G (somme des caps conteneurs ~14,25G / 15,6G RAM VPS). Ne PAS toucher au bareme C2 (invariant #5 : jamais de LLM dans le scoring).**
...
**Statut : TERMINE (2026-08-31 ; f57956a + 506edc4 + 86450a7, /deploy_verify SAIN x3). Pipeline /refonte_page complet : fiche admin.md §8 figee → handoff Claude Design (BRIEF-admin-D11 + maquette) → work_manager 8 lots + correctifs → deploy → revue design (6 ecarts confirmes + soldes) → 1 regression hover corrigee. Pur habillage front, 0 back / 0 modele / 0 migration. Reskin des 6 archetypes A→F + Apercu harmonise sur les 11 composants admin + charts/. NOUVEAU socle admin-local `assets/admin-table.css` (`.at-*` : tables denses / pills / chips cle-valeur / <details> / pagination + bascule data-label hybride au palier container 859px, et `.at-seg` segmente sans accent — PAS list-table.css, reserve Sets/Watchlist) + NOUVEAU helper `components/admin/AdminIcon.vue` (12 glyphes SVG inline currentColor, ZERO emoji). Correctif D22 acte : les courbes consomment les tokens `--chart-*` existants (CVD-valides, theme-flippants), pas de palette reinventee. 66 composants front (+AdminIcon). Correctifs de revue : bouton danger visible au repos (regle admin-locale, buttons.css intouche), pagination SERVEUR des set-flags via le param `offset` existant (file complete, 0 back), echelle Y des courbes (pas 1,5/2,5/3/4), etc. Survol des courbes restaure a la demande (tooltip valeurs, liste dans les « Absents » du brief mais voulu par l'usage). 741 tests front verts, eslint clean. Handoff + FIX archives dans docs/refonte-ui/handoff-admin-d11/.**

### Perimetre

- [x] Habillage homogene avec le reste de l'app : tokens `diggy-tokens.css`, reutilisation des composants partages (`TrackTable`, famille `charts/`, `AdminCard` des vues detail), zero couleur hardcodee.
- [x] Les 2 regimes visuels de l'Apercu (backlog en attente / a jour / inconnu) traites proprement.
- [x] Responsive mobile (barre d'onglets scrollable, cartes, tables, graphes).
- [x] Coherence visuelle des 6 onglets (densite, hierarchie, etats vide/chargement via `assets/page.css`).
...
**Statut : TERMINE — 2026-08-27 (ff0429c, deploy_verify SAIN, import OPS applique). Chantier LOCAL execute : ~381k sets TrackID indexes sans hydratation dans `trackid_index` (migration 0050) — spider 1 req/s (141 fenetres, 0 echec, reconciliation 3 voies propre), import OPS idempotent apres dump chiffre (381521 lignes, 40275 seedees hydrated, 12627 en 3507 clusters de-dup advisory, score NULL pour C12), Etape 0 arret backfill chrono. Produit le materiel de scoring de C12. Brief detaille : `docs/prompts/C11-index-trackid.md`. Issu de la consultation « Backfill TrackID » (3 iterations).**
...
**Statut : TERMINE (2026-08-30 ; 9566859, deploy_verify SAIN). Implementation des exigences A livree+deployee (8 lots) : enrich_priority + migration 0051 ; gate Beatport Tier-1-only (retries jamais floores, Deezer intact) ; verrou 2 stamping (phase du set / FLUX_PRIORITY=100, MAX-merge) ; swap curseur (backfill consomme trackid_index par score + beat 02h re-active) ; outil scoring local + import OPS --scores ; capture detail (migration 0052) ; alerte flux>budget. Pre-vol shadow-hydration complet -> net-new EXACT 1,15M, scope fige = phases 0-3 (~297k sets, ~1,1M titres, ~111j Beatport). OPS reliquat SOLDE (verifie prod 2026-09-04 : scores en prod, drain nocturne score-based actif, cap remonte 200->600). DIFFERE : C.1 recrawl polling hitRate (root cause confirmee 2026-09-04, voir la ligne C12 de la Vue d'ensemble) + graphe backlog front. Brief : `docs/prompts/C12-priorisation-backfill.md` ; spec pre-vol : `docs/prompts/C12-scoring-preflight-results.md`. A risque prod reel.**
...
- [x] C.1 recrawl re-base hitRate — LIVRE 2026-09-05 (go-forward + rattrapage) : `completion_pct` re-base sur `trackid_index.time_hit_rate` (decision : time_hit_rate, JOIN `trackid_index.set_id`), retrait de la finalisation is_id (`unidentified==0 -> final` + `new_pct>=1.0`), finalisation revue = `time_hit_rate >= RECRAWL_FINAL_HITRATE` (env 0.95) au pre-pass / stabilite `recrawl_count>=3` / plafond d'age `RECRAWL_MAX_AGE_DAYS` (env 730j = 24 mois, remplace >90j, cadence mensuelle 30j->cap). Rattrapage OPS AGRESSIF `scripts/backfill_completion_pct.py` (volet A re-base `completion_pct=time_hit_rate` ~40,5k sets ; volet B re-arm `final`->`active` + `recrawl_count=0` des ~39k incomplets < 0,95 ; dry-run defaut/`--apply`, dump-first, idempotent, `--volet a|b|both`). 1010 tests worker verts, ruff clean, aucune migration. Le « re-fetch detail SEULEMENT sur variation » (poll live du hitRate) reste HORS SCOPE : exigerait un endpoint listing par set inexistant ; la finalisation stabilite+age couvre le « stop apres N polls » (time_hit_rate = snapshot statique). RESTE OPS (William) : deployer L1 D'ABORD -> dry-run -> dump -> `--apply` -> re-dry-run -> surveiller CPU (le re-arm ~39k = churn set_tracks, cap `RECRAWL_MAX_SETS_PER_RUN` a piloter bas puis ramper, lecon hostinger-cpu-throttle). Impact monitoring attendu : le compteur admin « Sets a recrawler » (recrawl_status != final) bondit de ~1k a ~40k (correct, ils SONT a recrawler).
...
## C13 — Signaux de set & extracteur d'artiste depuis le titre

**Priorite : MOYEN**
**Estimation : 6-9 jours (dont ~1 j de corpus mining/benchmark gate, et un batch d'extraction LOCAL si le benchmark le justifie)**
**Depend de : C11 (`trackid_index` peuple = source des signaux + corpus des ~381k titres) ; recoupe l'Exigence B de C12 (audit de capture detail).**
**Statut : TERMINE 2026-09-10 — tous les sous-lots livres + deployes. C13.a (migration 0056 sets.channel/styles + funnel + affichage + backfill OPS applique) et C13.e (migration 0057 event_date/channel_canonical + filtre `?channel=` + chip cliquable) live. C13.b (mining `scripts/local/trackid_titles/` + jeu etiquete + benchmark) → GO/NO-GO trace : NO-GO LLM (C13.d) sur chiffres (rappel deterministe 76→91 %, residu ~4,5 %). C13.c extracteur SOUSTRACTIF (modules purs set_artist_extract/link/resolve) + verif Deezer + `link_set_artists` REECRIT (fin du ~60/42107 sous-chaine) cable au beat 05h30 ; precision durcie 4c427d0 (plancher fans + denylist media curee + fuites episode). Backfill artiste des ~53k sets existants execute en LOCAL (nuit 09→10/09, `worker/set_artist_backfill/`) : ~34 200 sets a DJ cliquable, 45 167 liens `set_artists` ; cap fil-de-l'eau releve 0→200. Issu de la consultation « infos TrackID » (2026-09). Absorbe le reliquat « Enrichissement set→artiste (D6 Sets liste) ». Residu ~15 % FP assume (channel = personne/label-patron) → piste future C13.f (source SC/YT).**

### Constat (verifie sur payload API reel, 2 sets sondes)

- Au **niveau du set**, `import_audiostream` (`server/api/trackid/importer.py:54`) ne persiste que **8 champs** sur `sets` : identite (`external_id`/`external_slug`/`source_url`), `title`, `duration_ms`, `played_date`, artwork, `can_reprocess` (C12). **Tout le reste du payload set est ignore sur la table `sets`.**
- Deux signaux set **utiles, deja collectes dans `trackid_index`** (le miroir du listing) mais **absents de `sets`** : `channel` et `styles` (genres donnes directement par TrackID, ex. `House, Tech House, Electro, Disco`). Les recuperer = surtout les **reporter sur `sets`** pour qu'ils soient exploitables avec le set.
- **`channel` = le nom de l'hebergeur sur la plateforme source** (chaine YouTube, compte SoundCloud), **PAS un artiste garanti** : parfois le DJ (`"Barry Can't Swim"`), parfois un label/media/festival (`"Hoax Productions"`, `"Boiler Room"`, `"Cercle"`). C'est le meilleur proxy disponible de « l'artiste du set », mais a traiter comme une chaine, pas comme un artiste sur.
- **TrackID n'expose AUCUN champ artiste au niveau du set.** L'artiste du set (le DJ) n'existe que dans `channel` et/ou noye dans le `title`. Pour l'obtenir de facon fiable, il faut le **DERIVER**.
- L'existant `link_set_artists` (`server/workers/tasks/artists.py:2280`) tente deja une derivation mais echoue quasi partout (**~60 sets sur 42 107** lies) : il n'est PAS dans le beat (declenchement manuel admin uniquement), ne matche que des artistes DEJA en base, et fait du sous-chaine brut sans confirmation.
- Signaux set additionnels a faible cout, deja dans `trackid_index` (optionnels, « on prend au cas ou ») : `timeHitRate`/`trackHitRate` (qualite d'identification), `favouriteCount`/`likeCount` (popularite), `status`/`audioStreamType`. Cote morceau, `referenceCount` (popularite d'un titre) est jete par `merge_tracklist` (cf. Exigence B de C12).

### C13.a — Persistance des signaux de set (back, low-cost, independant)

- [x] Migration : colonnes `sets.channel` (String) + `sets.styles` (StringArray) — et, si arbitre, un lot de signaux optionnels (`time_hit_rate`/`track_hit_rate`/`favourite_count`/`like_count`). « Ca pese rien, ce sera la en cas de besoin » = choix acte par William.
- [x] Ecriture au funnel : `import_audiostream` remplit `channel`/`styles` depuis le payload detail a chaque (re-)import (refresh gratuit au recrawl).
- [x] Backfill OPS des ~42k sets existants : `channel`/`styles` sont deja dans `trackid_index` → **report SQL sans re-crawl reseau** (join `sets.external_id = trackid_id::text`). Les signaux uniquement dans le detail (`referenceCount`…) necessiteraient un re-crawl → hors C13.a (a arbitrer separement, cf. C12 Exigence B).
- [x] Affichage : `channel` sur la fiche/liste Set (« chaine source »), `styles` en signal de genre.

### C13.b — Corpus mining + benchmark (GATE, read-only)

- [x] Miner les ~381k titres de `trackid_index` : tokeniser, remplacer chaque brique reconnue par un placeholder (`DATE`, `EP`, `FORMAT`, `DELIM`, `?`), **compter les squelettes** resultants (top ~40 = X % de couverture). Sortie : distribution + echantillons par squelette.
- [x] Constituer un **jeu de test etiquete a la main** (echantillon de titres → artiste(s) attendu(s)) pour mesurer precision/rappel.
- [x] **GO/NO-GO** : si le deterministe seul couvre ~85 % → pas de LLM. Si ~60 % avec une longue traine riche en artistes reels → le LLM devient interessant. **La necessite du LLM se decide sur ces chiffres, pas a priori.**

### C13.c — Extracteur deterministe (approche SOUSTRACTIVE, pas generative)

- [x] Ne PAS enumerer les formats de titres complets (explosion combinatoire) → reconnaitre les **~20 types de briques** ; n'importe quel ordre est gere automatiquement.
- [x] **Strippers de bruit** (petits, independants, testes unitairement) : dates (toutes formes : `05/09/2025`, `05092025`, `0509`, `20th Feb 2032`, `June 2026`, ISO…), marqueurs episodiques (`EP\d+`, `SET\d+`, `#\d+`, `Vol.\d+`, `Part \d+`, `\d{3,}` isole), mots de format (`DJ Set`, `Live`, `DJ Mix`, `Podcast`, `Guest Mix`, `Exclusive`, `FREE DOWNLOAD`, `Official`…), lieu/festival (gazetteer, best-effort).
- [x] **Connecteurs qui STRUCTURENT les artistes** (baliser, pas retirer) : `B2B`/`B3B`/`F2F`/`VS`/`X`/`&`/`feat.`/`ft.` → artistes autour + separation multi-artistes. Parentheses/crochets classes par contenu (date/lieu dedans = bruit ; nom dedans = a garder).
- [x] **Robustesse au format** : ne PAS gater sur « le titre commence par le nom » (perd `"Brooklyn - 20 juin - Artiste"`). Deux voies : (1) si `channel` matche un artiste reel → lien (independant du format du titre) ; (2) sinon, scan de tout le titre foldé pour un artiste connu, ou qu'il soit. Le « debut de titre » n'est qu'un **booster de confiance**, jamais un filtre.
- [x] **Filet fail-safe (invariant #4)** : l'extracteur PROPOSE, la verif DISPOSE. Tout candidat est confirme contre Deezer (matchers fold accents/ponctuation + plancher de fans, cf. X4) et/ou les artistes connus + `channel` AVANT tout lien `SetArtist`. Une extraction ratee → charabia → aucun match → **pas de lien** (l'erreur coute du rappel, pas de la precision). Reutiliser `resolve_name_to_id` (lookup-only, extrait de `get_or_create_artist`, cf. C12 Exigence B).
- [x] Cabler la derivation au beat / au funnel de sets (contrairement a `link_set_artists` qui ne tourne qu'en manuel), ou remplacer/ameliorer `link_set_artists`.

### C13.d — LLM sur le residu (CONDITIONNEL au benchmark C13.b)

- [ ] N'ajouter un LLM que si C13.b montre une traine residuelle grosse ET riche en artistes recuperables. Invariant #5 : extraction = tache frontiere-langage AUTORISEE ; le LLM ne calcule aucun score et n'ecrit JAMAIS en base — sa sortie **repasse par la verif Deezer** de C13.c.

### Decision VPS vs LOCAL (contrainte fair-use, lecon [[hostinger-cpu-throttle]] / AV10)

- **Sur le VPS (OK)** : C13.a (report SQL, negligeable), C13.b (mining read-only, ponctuel), C13.c deterministe (regex = CPU insignifiant), et l'extraction **fil-de-l'eau** des nouveaux sets (~600/j, residu ~60-90/j).
- **En LOCAL (obligatoire pour le batch LLM)** : le **backfill initial** d'extraction LLM sur le residu (~40-55k titres) sature les cœurs pendant **~5-22 h de CPU soutenu** → **exactement le profil qui a tripe le fair-use en aout (AV10)**, deja bride a 1 cœur pour Essentia. Donc, comme `worker/bpm_backfill/` et `worker/embedding_backfill/` : batch dans un conteneur Docker **sur le PC de William**, sortie NDJSON `{set_id, artiste_extrait}`, ecriture prod par un script OPS `import_set_artists_matches.py` (dry-run → dump → `--apply`, idempotent). La RAM n'est jamais le facteur limitant (un modele quantise ~1 Go tient dans les 15 GiB) — c'est **le CPU fair-use**.
- **Modeles open-weight candidats** (self-host, poids telechargeables, via llama.cpp GGUF Q4) : SmolLM2 (135M/360M/1.7B, Apache 2.0), Qwen2.5-Instruct (0.5B/1.5B/3B, Apache 2.0), Gemma 2 2B, Llama 3.2 1B/3B (licence a restrictions). Tache triviale (extraction courte) → un tres petit modele suffit. Alternative : Haiku cloud sur le seul residu (cout marginal), a mettre en balance avec le « zero dependance externe » du self-host (les titres sont publics → aucun enjeu de confidentialite).

### Definition of Done

- [x] `sets.channel` + `sets.styles` peuples (funnel + backfill des ~42k existants) et exposes cote UI.
- [x] Corpus mining livre (distribution des squelettes + jeu de test) + decision GO/NO-GO LLM tracee sur des chiffres.
- [x] Derivation d'artiste de set **automatique** (dans le beat/funnel) avec precision mesuree sur le jeu de test, liens `SetArtist` crees sous verif Deezer (0 faux positif tolere par design, invariant #4) → part des sets a DJ cliquable en forte hausse vs les ~60 actuels.
- [ ] Si LLM retenu : batch execute en LOCAL, ecrit en prod par script OPS (dump prealable), zero impact CPU VPS.

---

## C14 — Nouvelles sources de sets & veille artistes (YouTube/SoundCloud)

**Priorite : HAUT**
**Estimation : 10-15 jours en 3 volets separables (a : 3-4 j ∥ b : 5-7 j → c : 4-6 j apres gate corpus)**
**Depend de : C13 (chaine extracteur/resolution artiste reutilisee TELLE QUELLE : `set_artist_extract`/`set_artist_link`/`set_artist_resolve` + verif Deezer + `set_title_meta`) ; absorbe C6.d (suivi SoundCloud, reporte depuis 2026-07).**
**Statut : C14.a Phase 1 TERMINE + DEPLOYE 2026-09-22 (918c94e, /deploy_verify SAIN) — table `artist_cohort` (migration 0060) + recompute nightly (beat 04:15) + `check_followed_artists` branche sur la cohorte DUE (`select_due_cohort_ids`, budget-cap `COHORT_WATCH_NIGHTLY_BUDGET` + garde deadline AV9 ; `_check_new_sets` reste sur les follows) + admin `/admin/cohort` (`cohort_service` list/override) + onglet AdminCohort (7e ; pin/exclude/tier force + un-force via `forced_tier:null`). 1er recompute prod OK : T1=3469, T2=18336, total 21805. Brief `docs/prompts/C14a-veille-cohorte-deezer.md`. C14.b tranche « YouTube-first » TERMINE + DEPLOYE 2026-09-23 (5 commits 437b5bc->90e71bc, /deploy_verify SAIN x5, acceptance Schrotthagen VALIDE en prod : set YT cree + artiste 46469 lie) : entite `channels` (migration 0061) + client YouTube RSS/Data API + veille `crawl_youtube_channels` (01h30) + backfill `backfill_youtube_channel` + 8e onglet AdminChannels (add-by-URL/recherche/pre-selection cachee Redis) + source-awareness + fix pipeline nginx ; differes : resolution artiste->chaine en masse (Wikidata/MB), SoundCloud, reconciliation match_set, affinage extraction C13 sur titres YT. RESTE : C14.a Phase 2 (backfill discographie Deezer, outil LOCAL, GATED par benchmark) + C14.c (tracklists communautaires). Cadre 2026-09-18 (session de cadrage produit, chiffres verifies en prod le jour meme).**

### Constat (verifie prod 2026-09-18)

- **Le manque** : malgre les ~381k sets indexes TrackID, des sets attendus par les utilisateurs n'existent NULLE PART dans la base — parce qu'ils ne sont pas sur TrackID. Cas d'ecole verifie : **Schrotthagen** — l'artiste EST en base (id 46469, lie Deezer, 5 tracks catalog), **0 occurrence** dans les 381k du listing TrackID, alors que son set « Schrotthagen | Full Set at Ritter Butzke » est sur YouTube ; et **Ritter Butzke est un channel connu de la base** (8 sets importes, 36 au listing). Toute la chaine aval existe (extraction C13 aurait matche l'artiste) — c'est le ROBINET D'ENTREE qui manque.
- **Biais structurel TrackID** : le top channels de la base = radios (NTS 6 789, Rinse FM 3 650, The Lot Radio 3 501, Rinse France 2 121…) que la communaute soumet massivement ; les chaines de **lieux/evenements** sont sous-representees (Ritter Butzke 36/381k). Les sets manquants vivent exactement la ou TrackID est faible → la veille directe YT/SC a sa valeur maximale sur cette classe de chaines.
- **Cohortes artistes (mesure prod)** : ~15,6k DJs avec ≥1 set lie (post-C13) ; 9 759 avec ≥1 set/12 mois ; 2 173 a ≥3 sets/12m ; 971 a ≥5 sets/12m ; 1 689 « actifs ET profonds » (≥2 sets/12m ∩ ≥10 tracks catalog) ; lib users = 580 artistes (313 aussi DJs de set) ; suivis manuels = 13 ; **T1 naturel (lib ∪ suivis ∪ ≥3 sets/12m ∪ actifs-profonds) = 3 367** ; T2 large (≥10 tracks OU ≥1 set) = 25 279 ; total `artists` = 155 749. `event_date` couvre 28 724/81 847 roots — la recence retombe sinon sur `played_date` (date d'upload), qui est le BON signal pour une veille (« actif sur les plateformes »).
- **Faits externes verifies (2026-09-18)** : TrackID.net accepte des soumissions d'URL mais en **Premium payant** uniquement (decision : NON, voir ci-dessous). L'API SoundCloud a partiellement rouvert en 2026 (self-serve keys annoncees en mai, sinon formulaire a approbation lente) → **deposer la demande de cle DES MAINTENANT**, independamment du reste. 1001Tracklists : pas d'API publique, anti-scraping → ecarte.

### Decisions actees (cadrage 2026-09-18)

- **PAS de TrackID Premium** (soumission d'URL payante) : cher, peu important. Consequence ASSUMEE : les sets decouverts hors TrackID entrent en base **sans tracklist ou avec une tracklist partielle** — etat deja tolere par le produit (les sets TrackID sont eux-memes partiels par nature : seules les tracks identifiees sont stockees).
- **La decouverte est CHANNEL-CENTRIC** (colonne vertebrale) ; la veille par artiste est un COMPLEMENT. Une cohorte derivee de la base est structurellement aveugle aux artistes que TrackID n'a jamais vus (cas Schrotthagen) ; une chaine surveillee decouvre des sets ET des artistes inconnus (extraction C13 du titre + verif Deezer, creation gatee invariant #4). Les chaines qui comptent = quelques CENTAINES (borne), pas 15k artistes.
- **La selection d'artistes n'est pas une liste manuelle** : c'est un SEUIL sur une cohorte DERIVEE (signaux : lib/likes/follows, activite set + recence, profondeur catalog, prestige channel, fans Deezer) + **auto-promotion/retrogradation** au recalcul (un artiste qui sort un set, gagne un like ou apparait sur un gros channel entre SEUL ; un silencieux redescend). Les 13 follows manuels = epingles. Revue humaine ONE-SHOT en admin, pas d'entretien permanent.
- **Sets metadata-only acceptes** : `source='youtube'/'soundcloud'`, titre/chaine/date/duree/artwork/embed officiel, hors similarite/scoring tant que 0 track liee.
- **Tracklists communautaires** : description YouTube D'ABORD (structuree, 1 unit API), commentaires timecodes ENSUITE (« quelqu'un fait toujours le job » — genre tres codifie, ≥N lignes timecodees = detection quasi parfaite). Matching CONSERVATEUR (invariant #4) : ligne validee Deezer/Beatport → lien catalog ; ligne douteuse (« ??? », titre sans artiste) → TEXTE BRUT timecode, affichee mais non liee. Provenance `tracklist_source='community'` (autorite < trackid, esprit `bpm_source`). On ne remplace une tracklist que par une MEILLEURE (jamais de degradation) ; une apparition TrackID ulterieure SUPERSEDE (reconciliation `match_set`).
- **Backfill discographie Deezer** : one-shot par artiste (« complete une fois = plus besoin »), ordre de priorite, gate par BENCHMARK (net-new/artiste par tier), outil LOCAL (lecon AV10 : Deezer de masse interdit sur VPS).

### C14.a — Veille tracks Deezer sur cohorte derivee + backfill discographie

- [x] Materialiser la **cohorte derivee** (signaux ci-dessus) avec tiers de cadence (quotidien ~3,4k / hebdo / mensuel) + recalcul periodique (auto-promotion) + surface admin (voir la liste, pin/exclude, tier force).
- [x] Brancher le **check releases Deezer** sur la cohorte au lieu des seuls follows — machinerie `check_followed_artists` + `_crawl_track` REUTILISEE (feed Nouveautes inchange) ; budget par tier (~3,4k/j ≈ 56 min a 1 req/s en fenetre nocturne, ordre de grandeur `link_artists_deezer` → VPS OK).
- [ ] **BENCHMARK discographie (GATE)** : echantillon stratifie 30-50 artistes (lib / ≥5 sets / ≥10 tracks / traine) en dry-run local → req/artiste (~1 + nb albums ≈ 20-60), tracks ramenees, **% deja en base** (dedup `normalized_key`/ISRC) → **NET-NEW par artiste et par tier**. Decide : cohorte du backfill (couper la ou le net-new reste digeste) + projection d'inflow Beatport (capacite ~9 900/j — salves plafonnees, lecon sanity-check 2026-07-31) + impact taille du pool similarite/reco.
- [ ] **Outil LOCAL backfill discographie** (7e outil, moule `worker/set_artist_backfill/`) : artist/albums + album/tracks Deezer depuis IP residentielle → NDJSON → script OPS d'import (dry-run/dump/`--apply`, idempotent) ; `enrich_priority` BAS (sous les phases C12, jamais devant les tracks de sets) ; marqueur one-shot par artiste.

### C14.b — Entite channel & veille YouTube/SoundCloud (decouverte de sets ET d'artistes)

- [x] **Entite channel de 1re classe** : type (artiste/label/organisateur/radio), plateforme (youtube/soundcloud), external_id, `artist_id` NULLABLE (une chaine d'organisateur n'a pas d'artiste ; un artiste peut avoir plusieurs chaines). PAS un detournement de `watched_entities` (domaine playlists).
- [x] **Seed de la liste** : top-N `trackid_index.channel`/`sets.channel_canonical` (la base sait deja quels channels sont pertinents electro) + priorisation par « ratio de couverture TrackID » (channels sous-couverts = valeur max) + **curation one-shot admin de la classe lieux/collectifs** (William + utilisateurs) + boucle de croissance (set importe dont le channel n'est pas surveille → candidat en admin).
- [ ] **Resolution artiste→chaine SANS bruler le quota search** : Wikidata **P2397** (YouTube channel ID — Wikidata deja utilise pour le graphe de genres) + **MusicBrainz** (relations URL artistes : chaines YT/SC officielles) + match nom foldé (`workers/artist_names`) en heuristique secondaire ; search YouTube (100 units) en dernier recours ; curation traine en admin. En masse → drain LOCAL.
- [x] **Veille** (YouTube ; SoundCloud differe) : flux **RSS YouTube par chaine** (gratuit, sans quota — la veille de 2-3k chaines = ~1 h de HTTP leger/jour) + API SoundCloud (cle self-serve — demande a deposer immediatement) ; filtre duree >30-40 min = « c'est un set » ; anti-doublon vs `trackid_index`/`sets` AVANT creation.
- [x] **Sets metadata-only** (reconciliation match_set differee) : `source='youtube'/'soundcloud'` (aujourd'hui `sets.source` est toujours `'trackid'` — auditer les lecteurs), chaine C13 appliquee au titre (event_date, channel_canonical, extraction artiste + verif Deezer — REUTILISEE TELLE QUELLE, elle est source-agnostique) ; etats UI (« sans tracklist » / « tracklist partielle ») ; exclusion similarite/scoring tant que 0 track liee ; **reconciliation `match_set` si le set apparait plus tard sur TrackID (la version TrackID gagne)** ; dedup cross-source ULTRA-CONSERVATRICE (prudence C6/C11, invariant #4).

### C14.c — Tracklists communautaires (description + commentaires timecodes)

- [ ] **GATE corpus (read-only, style C13.b)** : sur ~200 sets connus, fetch description + commentaires (`videos.list` + `commentThreads`, ~2-3 units/video — negligeable) → mesurer (1) taux « une tracklist existe » (description vs commentaire), (2) taux de parse propre, (3) **delai d'apparition** du commentaire apres upload. **GO/NO-GO et dimensionnement sur ces chiffres, pas a priori.**
- [ ] **Parseur PUR** (stdlib regex, module type `set_title_meta`) : detection du commentaire-tracklist (≥N lignes commencant par un timecode), extraction timecode + libelle, tolerance `???`/`ID`/remix inconnus/lignes sans artiste.
- [ ] **Matching ligne→catalog via la machinerie existante** (search Deezer/Beatport + validation X3/X4 `_deezer_hit_matches`/matchers remix-aware) : ligne validee → `set_track` avec `catalog_id` ; ligne douteuse → `set_track` **TEXTE-SEUL** (`catalog_id` NULL, libelle brut + timecode — evolution de modele) ; `tracklist_source='community'` ; `completion_pct` = lignes matchees/totales (enfin un sens naturel pour cette source).
- [ ] **Re-visite en backoff** (jumelle `recrawl_incomplete_sets`) : check a la decouverte, +48 h, +7 j, +30 j, +90 j, cloture en etat final ; garde `CADENCE_SLACK_DAYS` (piege documente) ; remplacement uniquement par une meilleure version ; bouton « rechercher la tracklist » (re-visite a la demande sur un set cloture).

### Decision VPS vs LOCAL (lecon [[hostinger-cpu-throttle]] / AV10)

- **VPS OK** : veille Deezer budgetee (~3,4k req/j), poll RSS des chaines (HTTP leger), checks description/commentaires (~2-3 units/video), extraction C13 fil-de-l'eau des sets decouverts.
- **LOCAL obligatoire** : backfill discographie (Deezer de masse, IP residentielle) ; resolution artiste→chaine en masse si elle depasse la traine curable a la main.

### Definition of Done

- [x] Cohorte derivee materialisee (admin : liste, tiers, pin/exclude) + veille tracks Deezer active sur ~3,4k artistes — le feed Nouveautes alimente au-dela des 13 follows.
- [ ] Benchmark discographie livre (net-new/artiste par tier trace) + backfill execute sur la cohorte decidee, inflow Beatport sous controle (jamais devant les tracks de sets).
- [x] N chaines surveillees (seed + curation lieux) ; des sets YT/SC entrent en base en metadata-only avec artiste extrait/verifie — **le cas Schrotthagen (set Ritter Butzke) DOIT etre rattrape par le mecanisme, c'est le test d'acceptance** — VALIDE (YouTube) en prod 2026-09-23 : set YT cree + artiste 46469 lie ; volet SoundCloud differe.
- [ ] Gate corpus tracklists trace sur chiffres ; si GO : parseur + matching conservateur + re-visite backoff en prod ; part des sets communautaires avec tracklist (partielle) mesuree au monitoring.

---

## D12 — Pre-ecoute Beatport universelle (bouton + overlay embed)

**Priorite : MOYEN**
**Estimation : 2-3 jours (quasi front-only)**
**Depend de : rien (BeatportEmbed + AddModal + CSP frame-src deja en place). AUCUN modele ni migration.**
**Statut : TERMINE — 2026-09-22. Livre et deploye en 3 iterations le meme jour-et-demi : v1 overlay modal (c2391a0, 6 lots /work_manager L1 back 9 schemas+2 builders / L2 socle / L3 surfaces+fix / L4 Track Detail / L5 mappings vues / L6 builders list_catalog+artist — les 2 derniers attrapes par la verif rendu, invisibles aux 3 500 tests) → v2 barre dockee non-bloquante (30df211, L7 : navigation possible pendant l'extrait) → v3 carte compacte « remplace + chip reprendre » (6f277bc, L8 sur handoff Claude Design `_design/…/realign/BRIEF-beatport-bar.md` : carte centree min(608px) qui epouse l'embed, AUCUN texte, PlayerBar jamais co-visible, reservation 188px) + chore CI 9feeacd (retry smoke deploy apres un 502 flaky sur /api/artists/). /deploy_verify SAIN ×3, checklist humaine validee 2026-09-22, captures revue transmises a Claude Design. RESIDUELS (opportunistes, hors DoD) : ActivityAlbumCard non cable (rows par-track des releases groupees album) ; divergence pre-existante `bpm_source` emis par genre_service mais absent de GenreTrackItem (droppe par pydantic) ; commentaire perime « BeatportOverlay » BeatportEmbed.vue:21 (1 ligne) ; latence structurelle GET /api/artists/ ~6-12 s (HashAgg unnest(genres), candidate quick-win perf).**

### Constat (verifie prod 2026-09-21)

- Une track `has_preview=false` n'a AUCUN bouton play nulle part (le gate `v-if="has_preview"` est partout : TrackTable, TrackCard, Hub, Collections). Le seul acces a un extrait Beatport = l'encart inline `BeatportEmbed` sur le hero de Track Detail (fallback quand pas de preview Deezer).
- Volumetrie prod : **75 171** lignes catalog ont `beatport_id IS NOT NULL AND NOT has_preview` (**10,7 %** du catalog de 699 603) ; 554 162 (79,2 %) ont une preview Deezer ; 70 270 (10,0 %) n'ont ni l'un ni l'autre. → la feature fait passer la couverture de pre-ecoute de **79 % a ~90 %**, depuis TOUTES les listes.
- La contrainte licence est structurelle et deja respectee : l'embed officiel `embed.beatport.com` est le SEUL moyen licencie de jouer un extrait Beatport (le playback direct de `sample_url` est contractuellement exclu) ; l'iframe est une boite noire — JAMAIS de pont vers le store `audioPlayer` (commentaire canonique dans `BeatportEmbed.vue`).
- La donnee est deja largement exposee : `CatalogEntryOut` porte `beatport_id` (herite par SimilarTrackOut/RecommendationItem → Explorer, Radar, similar, reco l'ont deja dans le payload).

### Decisions actees (cadrage 2026-09-21)

- **Frontiere player** : ouvrir l'overlay Beatport **PAUSE** le player Deezer s'il joue (pause, pas `close()` — la barre reste, on reprend ensuite). Jamais deux audios. L'auto-advance de la file Deezer continue de **SKIPPER** les tracks Beatport-only (comportement actuel pour `has_preview=false`) : l'ecoute Beatport est un geste MANUEL single-shot, jamais dans la queue — pas d'avis depuis l'overlay, pas de progression, pas de volume (boite noire).
- **Bouton distinct** : meme position/taille que le play Deezer mais identite visuelle Beatport (glyphe/couleur, tooltip « Extrait Beatport ») pour que le comportement different (overlay, pas de lecture immediate) ne surprenne pas. Priorite Deezer quand les deux existent (`has_preview` → play Deezer ; sinon `beatport_id` → bouton Beatport ; sinon rien).
- **PAS d'autoplay** : impossible (iframe cross-origin sans API JS, bloque navigateur) et assume — flux a 2 clics (bouton → overlay charge → play dans l'iframe).
- **Track Detail : coherence totale** — l'encart inline `BeatportEmbed` du hero est SUPPRIME, remplace par le meme bouton-overlay que partout. Le lien « Voir sur Beatport ↗ » reste via `PlatformLink` dans `hero-links`.

### Lots

- [x] **L1 — `BeatportOverlay.vue`** : modale (chrome `AddModal` reutilise, variante bottom-sheet mobile) hebergeant l'embed officiel (reutiliser `BeatportEmbed` ou extraire son iframe — le lazy-mount IntersectionObserver n'a plus de sens dans une modale, charger a l'ouverture) + titre/artiste de la track + lien « Voir sur Beatport ↗ ». A l'ouverture : `audioPlayer` mis en PAUSE. Etat global leger (petit store ou provide/inject depuis App.vue) pour qu'UN SEUL overlay serve toutes les vues.
- [x] **L2 — Boutons dans les surfaces** : `TrackTable` (Explorer/Radar), `TrackCard` (6 vues detail via prop), sections Hub (`HubSearchResults`/Trends/Reco/Activity), `CollectionDetailView` — le gate `v-if="has_preview"` devient le ternaire Deezer/Beatport/rien. Invariant NavCover respecte (bouton = controle interactif AU-DESSUS du lien etire). Etats visuels du bouton Beatport (hover, focus-visible) sur tokens.
- [x] **L3 — Track Detail** : retrait de l'encart inline `BeatportEmbed` du hero → bouton-overlay a la place (meme logique que le HeroPlayer Deezer cote placement). `PlatformLink` Beatport conserve.
- [x] **L4 — Back leger (si besoin)** : audit des schemas de rows secondaires (`SetTrackOut`, `AlbumTrackOut`, activity, collections, random-track) → ajouter `beatport_id` la ou il manque pour que le bouton apparaisse aussi sur ces surfaces. Champ additif, aucun modele ni migration.
- [x] **Verif RENDU headless** avant commit (light+dark, desktop+mobile, overlay ouvert/ferme, variante bouton dans les listes) — vitest ne voit ni l'overlay ni les variantes visuelles.

### Definition of Done

- [x] Toute surface qui affiche un play Deezer affiche le bouton Beatport pour une track `!has_preview && beatport_id` ; clic → overlay avec l'embed, player Deezer en pause.
- [x] L'encart inline de Track Detail a disparu ; le hero porte le meme bouton.
- [x] File Deezer : auto-advance inchange (skip des Beatport-only), aucun pont audio avec l'iframe.
- [x] CSP `frame-src embed.beatport.com` verifiee effective sur toutes les pages (header global, pas scope Track Detail).
- [x] Tests front verts + verif rendu headless conforme.
...
| ~~**Enrichissement set→artiste (D6 Sets liste)**~~ → **PROMU en chantier C13** (« Signaux de set & extracteur d'artiste depuis le titre »). Constat de depart : ~60 sets sur 42 107 lies a un `SetArtist` (matcher `tasks/artists.py` manuel, artistes deja en base + sous-chaine verbatim). Voir C13 pour le cadrage complet (report `channel`/`styles`, corpus mining, extracteur soustractif, verif Deezer, batch LLM en local). | Absorbe par C13 |
...
| C10 | Pool similarite precalcule nightly | CADUC — supersede par C9.c retrieval-first (plus de full-pool a cacher) | AV3 (palliatifs + mesures) |
