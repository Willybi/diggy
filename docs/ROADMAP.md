...
**Statut : A CADRER — issu de la consultation « infos TrackID » (2026-09). Absorbe le reliquat « Enrichissement set→artiste (D6 Sets liste) ». A risque prod modere (migration + liens SetArtist).**
...
- [ ] Migration : colonnes `sets.channel` (String) + `sets.styles` (StringArray) — et, si arbitre, un lot de signaux optionnels (`time_hit_rate`/`track_hit_rate`/`favourite_count`/`like_count`). « Ca pese rien, ce sera la en cas de besoin » = choix acte par William.
- [ ] Ecriture au funnel : `import_audiostream` remplit `channel`/`styles` depuis le payload detail a chaque (re-)import (refresh gratuit au recrawl).
...
- [ ] Affichage : `channel` sur la fiche/liste Set (« chaine source »), `styles` en signal de genre.
