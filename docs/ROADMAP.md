...
----  ------------------------------------  ----------  ----------   ------
 ...
 X3   Fiabilite matching enrichissement    MOYEN       3-5 jours    EN COURS — prevention X3.a/b deployee (2026-07-22, bedd997) ; reste rollout X3.c (script livre, --apply en attente)
...
**Statut : EN COURS — prevention X3.a (Deezer) + X3.b (Beatport) LIVREE et DEPLOYEE le 2026-07-22 (commit bedd997, /deploy_verify SAIN). Reste X3.c : le script `scripts/reverify_platform_ids.py` est livre (dry-run/`--apply`, idempotent) mais le rollout `--apply` (reparation des lignes existantes) est en attente d'une decision ops (dry-run de mesure + dump prealable). Watch recall au 1er nightly.**
...
| **MinIO memoire — mitigations posees 2026-07-22, A VERIFIER** : mem montee 90,6 %→97,56 % de 1 GiB en ~1 j (container up 11 j, jamais restart hors OOM ; sans lien avec X3). Poses : (1) `mem_limit` 1G→**2G** dans `docker-compose.yml` (prend effet au prochain deploy — `up -d` recree le container ; un simple `restart` ne relit PAS la limite) ; (2) cron VPS **restart hebdo `minio`** lundi 00:30 Europe/Paris (`CRON_TZ` + `30 0 * * 1`, log `/var/log/diggy-minio-restart.log`). RESTE : re-mesurer `docker stats minio` a J+1 apres un restart — remontee LENTE vers ~98 % = working-set (2G suffit) ; remontee RAPIDE malgre 2G = vraie fuite a investiguer (version MinIO). | A verifier a J+1 apres 1er restart / apres deploy du 2G |
