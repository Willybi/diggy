...
| **MinIO proche de sa limite memoire** : au /deploy_verify X3 (2026-07-22) le container `minio` tournait a 90,6 % de sa limite (928 MiB / 1 GiB, up 10 j) — observation ponctuelle, pre-existante, sans lien avec X3. MinIO bufferise volontiers (distinguer cache normal vs fuite) ; risque d'OOM-kill si la limite est atteinte. Action = surveiller `docker stats minio`, envisager de relever `mem_limit` ou verifier la retention. | Opportuniste — surveiller ; agir si OOM ou montee continue |
