...
----  ------------------------------------  ----------  ----------   ------
 ...
 D12  Pre-ecoute Beatport universelle (bouton + overlay embed) MOYEN 2-3 jours A FAIRE — cadre 2026-09-21 (session produit, volumetrie prod du jour) : partout ou un bouton play Deezer existe, une track `!has_preview && beatport_id` (75 171 lignes = 10,7 % du catalog → couverture pre-ecoute 79 %→~90 %) affiche un bouton play VARIANTE Beatport ouvrant un OVERLAY hebergeant l'embed officiel (seul moyen licencie, iframe boite noire, PAS d'autoplay — impossible et assume) ; l'encart inline de Track Detail est REMPLACE par le meme bouton (coherence totale). Decisions : ouvrir l'overlay PAUSE le player Deezer (jamais 2 audios) ; l'auto-advance de la file continue de SKIPPER les Beatport-only (ecoute manuelle, jamais dans la queue) ; priorite Deezer si les 2 existent. Quasi front-only, AUCUN modele ni migration
```
...
**Statut : A FAIRE — cadre 2026-09-21 (session produit, volumetrie verifiee en prod le jour meme).**
...
- [ ] **L1 — `BeatportOverlay.vue`** : modale (chrome `AddModal` reutilise, variante bottom-sheet mobile) hebergeant l'embed officiel (reutiliser `BeatportEmbed` ou extraire son iframe — le lazy-mount IntersectionObserver n'a plus de sens dans une modale, charger a l'ouverture) + titre/artiste de la track + lien « Voir sur Beatport ↗ ». A l'ouverture : `audioPlayer` mis en PAUSE. Etat global leger (petit store ou provide/inject depuis App.vue) pour qu'UN SEUL overlay serve toutes les vues.
- [ ] **L2 — Boutons dans les surfaces** : `TrackTable` (Explorer/Radar), `TrackCard` (6 vues detail via prop), sections Hub (`HubSearchResults`/Trends/Reco/Activity), `CollectionDetailView` — le gate `v-if="has_preview"` devient le ternaire Deezer/Beatport/rien. Invariant NavCover respecte (bouton = controle interactif AU-DESSUS du lien etire). Etats visuels du bouton Beatport (hover, focus-visible) sur tokens.
- [ ] **L3 — Track Detail** : retrait de l'encart inline `BeatportEmbed` du hero → bouton-overlay a la place (meme logique que le HeroPlayer Deezer cote placement). `PlatformLink` Beatport conserve.
- [ ] **L4 — Back leger (si besoin)** : audit des schemas de rows secondaires (`SetTrackOut`, `AlbumTrackOut`, activity, collections, random-track) → ajouter `beatport_id` la ou il manque pour que le bouton apparaisse aussi sur ces surfaces. Champ additif, aucun modele ni migration.
- [ ] **Verif RENDU headless** avant commit (light+dark, desktop+mobile, overlay ouvert/ferme, variante bouton dans les listes) — vitest ne voit ni l'overlay ni les variantes visuelles.

### Definition of Done

- [ ] Toute surface qui affiche un play Deezer affiche le bouton Beatport pour une track `!has_preview && beatport_id` ; clic → overlay avec l'embed, player Deezer en pause.
- [ ] L'encart inline de Track Detail a disparu ; le hero porte le meme bouton.
- [ ] File Deezer : auto-advance inchange (skip des Beatport-only), aucun pont audio avec l'iframe.
- [ ] CSP `frame-src embed.beatport.com` verifiee effective sur toutes les pages (header global, pas scope Track Detail).
- [ ] Tests front verts + verif rendu headless conforme.
