...
----  ------------------------------------  ----------  ----------   ------
 ...
 D6   Refonte UI listes + Radar + transverses BAS      8-12 jours   EN COURS — Explorer (p.1) 2026-07-21 + Radar (p.2) + Hub (p.3) 2026-07-23 + Sets liste (p.4) 2026-07-24 + Playlists liste (p.5) 2026-07-27 + Artistes liste (p.6) 2026-07-28 + Genres liste (p.7) 2026-07-31 + Genre Detail (p.8) 2026-08-04 ; les 8 pages livrees, restent D6.0 Suppression Rating + revue design Phase 5 Genre Detail
...
- [ ] CHECK J+2 (21/07) : passe1=… passe2=… backlog=… (doit BAISSER vs 21 114)
...
**Statut : A FAIRE — proposition William 2026-08-03 ; lancer par N3.0 (dimensionnement GO/NO-GO)**
...
**Statut : A FAIRE — cadre le 2026-07-31 (session monitoring).**
...
- [ ] Script local : ~500 entrees AVEC bpm/key Beatport ET preview → analyse des 30 s → mesures : ecart BPM (avec repli d'octave x2/÷2 dans une plage plausible 70-180), taux d'accord key (exact + voisins Camelot ±1/relative)
- [ ] Seuils GO/NO-GO a fixer et documenter (proposition : BPM >= 95 % apres repli d'octave ; key >= 75 % exact-ou-voisin)
- [ ] Le benchmark fige la stack (Essentia seul, ou mix librosa/libkeyfinder) et les parametres

### E2.b — Industrialisation (backfill + fil de l'eau)

- [ ] Backfill des ~49k : batch-commit, reprise sur interruption, throttle poli du telechargement des previews (~20 GB au total)
- [ ] Provenance : `bpm_source='analysis'` / `key_source='analysis'` ; ne JAMAIS ecraser une valeur rekordbox/beatport/deezer existante ; un enrichissement Beatport ulterieur ECRASE la valeur analysis (hierarchie invariant #3)
- [ ] Fil de l'eau : une entree qui gagne une preview sans lien Beatport devient candidate (hook post-enrichissement Deezer ou sweep periodique budgete, pattern E1)
- [ ] AUCUNE migration attendue (`bpm`/`key`/`bpm_source`/`key_source` existent)

### E2.c — Frontend (mineur, optionnel v1)

- [ ] Signaler la valeur estimee (ex. tooltip/affichage discret la ou bpm/key sont rendus) — a cadrer ; les filtres Explorer/Radar remontent ces valeurs sans changement
...
**Statut : A FAIRE — cadre le 2026-07-31. Horizon recherche assume (C9.d).**
...
- [ ] Migration pgvector + schema versionne par modele (`model_name`/`model_version` — changer de modele = re-embedder, pas de vecteurs heterogenes melanges)
...
- [ ] Endpoint voisins par `catalog_id` (perimetre `catalog_visible`, cache Redis, pattern `GET /api/sets/{id}/similar`)
- [ ] Shelf « Sonne comme » sur Track Detail (consomme TrackCard, aucune nouvelle famille de composants attendue)
...
**Statut : A FAIRE — inscrit 2026-08-03, retour d'usage William sur Genre Detail fraichement livree.**
