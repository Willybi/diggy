...
- **`<ScoreRing mode="pct">` (2026-07-17, chantier Set Detail)** : spec additive livrée (BRIEF-set-detail.md) — `mode 'score'|'pct'`, défaut `'score'` bit-à-bit identique ; mode % = arc proportion + « N % » au centre (espace fine insécable), md recommandé. **1ʳᵉ migration de la cible « RingPct → géométrie ScoreRing »** posée plus haut ; la migration des autres usages RingPct reste un chantier ultérieur. **Note (2026-07-24, chantier liste `/sets`)** : la refonte a d'abord posé un `<ScoreRing mode="pct">` sur la colonne « Tracks » de la liste, puis l'a **retiré au test** — l'import de sets ne stocke QUE les tracks identifiées (`identified == total` pour 100 % des sets listables, vérifié en prod : 0 placeholder « ID », 0 track sans catalogue) → le `%` valait **structurellement toujours 100 %**, sans information (même diagnostic que la colonne Source). La colonne affiche donc désormais le **nombre de tracks** (compteur mono), pas un anneau. Conséquence : **`<RingPct>` perd son dernier consommateur** (SetsView) → composant orphelin, nettoyage/migration à trancher ultérieurement. La migration « RingPct → géométrie ScoreRing » reste donc portée par Set Detail (1er point ci-dessus) ; aucun usage RingPct **ni** ScoreRing-pct sur la liste `/sets`.
- **`<ScoreRing>` en conso bi-score (2026-07-22, chantier Radar)** : Radar consomme le composant tel quel (`size="sm"` 30 px, `mode="score"`, géométrie NON redéfinie) en **deux colonnes côte à côte** (Tendance / Pour toi), score `*_score_10 / 10`. Conventions posées par le handoff Radar, réutilisables par toute liste bi-score : **colonne de tri active surlignée** (bande verticale `--accent-wash` en-tête + cellules), **score absent = « — » muet** (pas d'anneau à 0), accent **velocity ▲** optionnel en coin de l'anneau. Aucune modification du composant.
- **`<DiscoveryCard>` (2026-07-23, chantier Hub)** : NOUVEAU composant = la variante **card** de découverte restée à cadrer ci-dessus. Spec livrée (`docs/refonte-ui/handoff-hub/BRIEF-composants-hub.md`) — carte **horizontale** (cover `<Artwork>` 64 px + corps 3 lignes : titre · artistes · méta mono `BPM · KEY · âge`), **une anatomie / 5 variantes par props** (tendance `#rank` · reco `in_lib` · nouveauté · set · lien externe), méta dégradée sans tiret (champs nuls omis, âge tombe en premier), âge en **token brut** (`6 j`/`2 sem`, pas « Sorti il y a… »), badge contextuel monochrome accent, play hover-reveal desktop + toujours visible < 640, `<Artwork>` consommé non re-designé. **NE PAS confondre avec `<TrackCard>`** (ligne dense, distinct). Cohabite dans la même grille que `ActivityAlbumCard` (calée sur le même look). Consommateurs : 3 étagères du Hub, futur `/new-releases`, cards Radar éventuelles. 1re implémentation = chantier Hub. Le wrapper `<DiscoveryShelf>`/`<DiscoveryList>` (en-tête + grille) reste optionnel (structure de page si non extrait).
- **`<SetCard>` (2026-07-17, chantier Set Detail)** : NOUVEAU composant réutilisable (`docs/refonte-ui/handoff-set-detail/SPEC-set-card.md`) — carte set verticale (Artwork card, titre clamp 2, artistes, méta mono date · durée · N tracks, nulls omis), slot `#footer` d'extension (score, badges — vide par défaut). 1ʳᵉ conso : « Sets similaires » de Set Detail. **Correction (2026-07-23, pré-vol chantier liste `/sets`)** : la liste `/sets` **reste un TABLEAU enrichi** (décision William, fidèle à la fiche `sets-list.md` §5 + cohérente avec sa jumelle Playlists) — `<SetCard>` n'y est donc **PAS** réutilisé ; il reste le composant carte des « Sets similaires ». (La ligne d'origine annonçait à tort une réutilisation « telle quelle » sur `/sets`, or `<SetCard>` ne porte ni genre, ni source, ni avis, ni % — il ne pouvait pas satisfaire la row de la fiche.)
