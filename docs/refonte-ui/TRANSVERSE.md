...
- **`<ScoreRing mode="pct">` (2026-07-17, chantier Set Detail)** : spec additive livrée (BRIEF-set-detail.md) — `mode 'score'|'pct'`, défaut `'score'` bit-à-bit identique ; mode % = arc proportion + « N % » au centre (espace fine insécable), md recommandé. **1ʳᵉ migration de la cible « RingPct → géométrie ScoreRing »** posée plus haut ; la migration des autres usages RingPct reste un chantier ultérieur. **2ᵉ point de migration (2026-07-24, chantier liste `/sets`)** : la colonne « Tracks (%) » de la liste des sets consomme `<ScoreRing mode="pct" size="md">` (`score = identified_tracks / total_tracks`) à la place de `<RingPct>` — le brief Design décrivait cette géométrie (40 px, % centré, espace fine) ; ScoreRing-pct évite la coche-à-100 % et les tiers `--warn` de RingPct, et le % centré rend inutile le masquage responsive du libellé. `RingPct` reste en place sur ses autres usages (liste Playlists, détails) jusqu'à leur propre refonte.
