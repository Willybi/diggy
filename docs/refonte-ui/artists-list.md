...
`GET /api/artists/` gagne un param `genre` (nom brut) en **présence ≥ 1 track** : semi-join `catalog_artists → catalog` via la subquery `id_filter_query` (posée sur `base_query` ET `id_filter_query` pour que lignes/total/pillarCounts concordent, jamais une liste d'ids matérialisée — cap asyncpg 32767), `catalog_visible` inclus. **Orthogonal au filtre PILIER** (`FamilyChips`) : c'est le genre fin, pas la famille. Filtre = **appartenance brute** (≥ 1 track), PAS l'ensemble seuillé 20 % des chips affichées → un artiste à un seul track du style matche. Front : `genreFilter` (ref, URL-syncée `genre` via `useUrlSync`) + **chip retirable** `<FilterChip>` dans la `tools-row` (avant `FamilyChips`) + `extraParams` + `opinionOneShot.buildParams`. Landing depuis Genre Detail « Voir tous les artistes de ce genre » → `/artists?genre=`. Pas de picker (arrivée contextuelle). Aucune migration.
