...
4. **Tracks** : rangées alignées sur **Explorer, colonne genre en moins** (cover + **in-lib**, titre, artiste, BPM, Key, durée, play) → réutilise le composant de rangée Explorer / `<TrackCard>` + `<Artwork>`. Garde search + sort + toggle En bib + ~~infinite scroll~~ **aperçu borné** (cf. §5, amendé D8.b — la suite se consulte dans Explorer pré-filtré).
...
- **Shelves** Artistes / Sets / Playlists : gardés.
  - **AMENDÉ 2026-08-17 (D8.b)** : le « Voir les N autres » de chaque shelf ne fait plus d'**append inline** (Sets/Playlists) ni de **mode déplié paginé** (Artistes = ex-`ExpandableShelf`) → il **NAVIGUE** vers la liste pré-filtrée par genre — `/sets?genre=`, `/playlists?genre=`, `/artists?genre=` (RouterLink, chip retirable côté liste ; grammaire Hub top-9 → destination). Le shelf devient un **aperçu borné** (`SHELF_PREVIEW=8`). Libellé **sans compteur** (« Voir tous les sets de ce genre ») : la destination /sets & /playlists filtre en **dominance ≥ 25 %** (`GENRE_MIN_SHARE_PCT`, cohérent avec le filtre Style /sets calibré en D6) alors que le compteur du shelf est en **présence** → un « N » y serait faux ; /artists filtre en **présence ≥ 1 track** (cohérent avec son shelf). Le shelf Artistes quitte `ExpandableShelf` (composant partagé, **laissé intact**) pour une grille locale `.artists-grid` + `<RelBlock>` + RouterLink.
