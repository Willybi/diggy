...
- **Pastille cadence/fraîcheur → RETIRÉE de la liste** (décision William, 2 itérations de recette). Historique : `Quotidien/Hebdo/Mensuel` (ambigu, mot de fréquence pour une donnée d'activité) → relabel « fraîcheur brute » `MAJ X j` → **retrait complet**. Raison de fond : la fraîcheur (dernière nouveauté, `last_changed_at`) est **redondante** avec la colonne « Dernier crawl » dans les données actuelles (un changement est détecté *pendant* un crawl → les 2 dates coïncident presque toujours ; elle n'ajouterait de l'info que si elles **divergeaient** — « crawlée récemment mais MAJ il y a 40 j » = source qui s'endort —, ce qui n'arrive pas avec 8/56 playlists datées, toutes récentes). En prime, le rendu 2 lignes (quand la pastille existait) **décentrait le bouton Crawl** et son seul feedback était un tooltip natif discret. → pastille supprimée, **toutes les rows en 1 ligne centrée**. `last_changed_at` reste exposé par le back (inoffensif, réutilisable). **Reliquat** : un vrai indicateur « dormante/stale » pourra revenir **post-ouverture**, quand les données divergeront et le justifieront. Appliqué au **2ᵉ lot correctif Phase 6**.
